...
from schemas.models import (
    ModelResponse, ModelListResponse, ModelPromoteRequest,
    PaperSessionResponse, PaperSessionListResponse,
    StartPaperSessionRequest, ValidationResultResponse
)
...
async def list_models(
    ...
    try:
        models = model_service.get_all_models(db, stage=stage, experiment_id=experiment_id)
...
async def promote_to_paper(
    ...
    if model.stage != "staging":
        raise HTTPException(status_code=400, detail=f"Model must be in staging, currently {model.stage}")
...
async def promote_to_production(
    ...
    if model.stage != "paper":
        raise HTTPException(status_code=400, detail=f"Model must be in paper stage, currently {model.stage}")
    
    # Verify 2FA token
    if not request.totp_token:
        raise HTTPException(status_code=400, detail="2FA token required for production promotion")
    ...
    try:
        promoted = model_service.promote_model(db, model_id, "production", totp_token=request.totp_token)
...
async def rollback_production(
    model_id: int,
    to_model_id: Optional[int] = Query(None, description="Target model ID to rollback to"),
    ...
    if model.stage != "production":
        raise HTTPException(status_code=400, detail="Only production models can be rolled back")
...
async def start_paper_session(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start paper session: {str(e)}")


@router.post("/{model_id}/paper-sessions/{session_id}/stop", response_model=PaperSessionResponse)
async def stop_paper_session(
    ...
    if session.model_id != model_id:
        raise HTTPException(status_code=400, detail="Session does not belong to this model")
    ...
    try:
        ...
        await ws_channels.broadcast_paper_session_update(
            ...
            metrics={
                ...
                "sharpe_ratio": float(stopped.sharpe_ratio) if stopped.sharpe_ratio else None,
                "max_drawdown": float(stopped.max_drawdown) if stopped.max_drawdown else None,
            }
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to stop paper session: {str(e)}")
