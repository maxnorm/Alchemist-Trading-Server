...
def create_experiment(db: Session, experiment_data: ExperimentCreate) -> ExperimentResponse:
        ...
        for json_field in ["features", "currency_pairs", "hyperparameters"]:
            if row_dict.get(json_field) and isinstance(row_dict[json_field], str):
                try:
                    ...
                    row_dict[json_field] = [] if json_field in ["features", "currency_pairs"] else {}
        experiments.append(ExperimentResponse(**row_dict))
    
    return experiments


def get_experiment_by_id(db: Session, experiment_id: int) -> Optional[ExperimentResponse]:
...
def clone_experiment(db: Session, experiment_id: int, new_name: str) -> Optional[ExperimentResponse]:
    """Clone an experiment"""
