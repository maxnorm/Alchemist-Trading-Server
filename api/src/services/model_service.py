...
from schemas.models import (
    ModelResponse, PaperSessionResponse, ValidationResultResponse
)
...
def get_all_models(
    ...
    for row in rows:
        ...
        # Parse JSON fields
        for json_field in ["features", "hyperparameters", "metrics", "paper_trading_results"]:
...
def get_model_by_version(db: Session, version: str) -> Optional[ModelResponse]:
    ...
    result = db.execute(
        text("SELECT * FROM models WHERE version = :version"),
        {"version": version}
    )
    row = result.fetchone()
...
def promote_model(
    ...
    if target_stage == "production":
        ...
        if not validation['passed']:
            raise ValueError(f"Model validation failed: {', '.join(validation['messages'])}")
...
def rollback_production(
    ...
    if target_model_id:
        ...
        result = db.execute(
            text("SELECT * FROM models WHERE stage = 'archived' ORDER BY promoted_at DESC LIMIT 1")
        )
        row = result.fetchone()
        if row:
            row_dict = dict(row._mapping)
...
def start_paper_session(
    ...
    if WS_AVAILABLE:
        try:
            import asyncio
            asyncio.create_task(ws_channels.broadcast_paper_session_update(
                model_id=model_id,
...
def get_paper_session(db: Session, session_id: int) -> Optional[PaperSessionResponse]:
    ...
    result = db.execute(
        text("SELECT * FROM paper_trading_sessions WHERE id = :id"),
        {"id": session_id}
    )
    row = result.fetchone()
...
def stop_paper_session(db: Session, session_id: int) -> PaperSessionResponse:
    ...
    if updated_session:
        ...
        results = {
            ...
            "win_rate": updated_session.winning_trades / updated_session.total_trades if updated_session.total_trades > 0 else 0.0,
            "pnl": float(updated_session.pnl),
            "sharpe_ratio": float(updated_session.sharpe_ratio) if updated_session.sharpe_ratio else None,
            "max_drawdown": float(updated_session.max_drawdown) if updated_session.max_drawdown else None,
            "days_traded": (updated_session.ended_at - updated_session.started_at).days if updated_session.ended_at else 0,
            "trade_count": updated_session.total_trades
        }
        
        db.execute(
            text("UPDATE models SET paper_trading_results = :results WHERE id = :model_id"),
