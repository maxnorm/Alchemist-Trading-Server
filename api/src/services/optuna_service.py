...
from schemas.hyperparameters import OptunaSearchCreate, OptunaStudyResponse, TrialResponse
import json
import logging
...
def create_optuna_study(db: Session, search_data: OptunaSearchCreate) -> OptunaStudyResponse:
...
def get_study_by_id(db: Session, study_id: int) -> Optional[OptunaStudyResponse]:
    ...
    result = db.execute(
        text("SELECT * FROM optuna_studies WHERE id = :id"),
        {"id": study_id}
    )
    row = result.fetchone()
...
def get_study_by_experiment_id(db: Session, experiment_id: int) -> Optional[OptunaStudyResponse]:
    """Get Optuna study by experiment ID"""
    result = db.execute(
        text("SELECT * FROM optuna_studies WHERE experiment_id = :experiment_id ORDER BY created_at DESC LIMIT 1"),
...
def get_trials_by_study_id(db: Session, study_id: int) -> List[TrialResponse]:
    ...
    result = db.execute(
        text("SELECT * FROM optuna_trials WHERE study_id = :study_id ORDER BY trial_number"),
        {"study_id": study_id}
    )
