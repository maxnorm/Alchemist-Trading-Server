...
def get_model_equity_curve(
    ...
    try:
        ...
        return EquityCurveResponse(model_version=model_version, data=data, total_points=len(data))
    except Exception as e:
        logger.warning(f"Failed to get model equity curve: {e}")
        return EquityCurveResponse(model_version=f"model_{model_id}", data=[], total_points=0)
...
def get_performance_breakdown(
    ...
    try:
        ...
        breakdown = []
        for row in rows:
            breakdown.append(PerformanceBreakdownResponse(
                period=str(row[0]),
