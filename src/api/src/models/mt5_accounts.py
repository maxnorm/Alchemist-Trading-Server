...
class MT5Account(Base):
    ...
    # Relationships
    assignments = relationship("AccountModelAssignment", back_populates="account", cascade="all, delete-orphan")
    connections = relationship("MT5Connection", back_populates="account", cascade="all, delete-orphan")
...
class AccountModelAssignment(Base):
    ...
    assigned_by = Column(Integer, nullable=True)  # dashboard_users.id (nullable for now)
...
class MT5Connection(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    account_id = Column(Integer, ForeignKey("mt5_accounts.id", ondelete="CASCADE"), nullable=False, index=True)
