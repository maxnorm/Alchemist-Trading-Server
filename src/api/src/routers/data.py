...
async def get_quarantine_ticks(
    ...
    start_date: Optional[datetime] = Query(
        None, description="Filter by start date (datetime)"
    ),
    end_date: Optional[datetime] = Query(
        None, description="Filter by end date (datetime)"
    ),
    ...
    try:
        ...
        query += (
            " ORDER BY quarantined_at DESC, datetime DESC LIMIT :limit OFFSET :offset"
        )
        params["limit"] = limit
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch quarantine ticks: {str(e)}"
        )
