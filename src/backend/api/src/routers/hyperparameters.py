...
async def start_optuna_search(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to start Optuna search: {str(e)}"
        )
...
async def get_experiment_optuna_status(
    ...
    if not study:
        raise HTTPException(
            status_code=404,
            detail=f"No Optuna study found for experiment {experiment_id}",
        )
    return study
...
async def stop_search(
    ...
    if study.status != "running":
        raise HTTPException(
            status_code=400, detail=f"Study is not running (status: {study.status})"
        )
