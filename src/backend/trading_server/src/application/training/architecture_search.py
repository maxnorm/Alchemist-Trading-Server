...
class ArchitectureSearch:
    ...
    def suggest_architecture(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        for i in range(n_lstm_layers):
            units = trial.suggest_categorical(f"lstm_units_{i}", [64, 128, 256, 512])
        ...
        for i in range(n_dense_layers):
            units = trial.suggest_categorical(f"dense_units_{i}", [32, 64, 128, 256])
    ...
    def optimize(
        ...
        self.study.optimize(objective_func, n_trials=n_trials, show_progress_bar=True)

        self.logger.info(
            f"Architecture search complete. Best value: {self.study.best_value:.4f}"
        )
