    ...
    def _create_strategy(self) -> tf.distribute.Strategy:
        ...
        if self.strategy_type == "mirrored":
            ...
            if self.cluster_resolver:
                ...
                self.logger.warning(
                    "MultiWorkerMirroredStrategy requires cluster_resolver"
                )
                return tf.distribute.MirroredStrategy()
            ...
            self.logger.warning(
                f"Unknown strategy type: {self.strategy_type}, using default"
            )
            return tf.distribute.get_strategy()

    def create_agent_within_strategy(
        self, state_shape: tuple, action_size: int, **agent_kwargs
    ) -> DQNAgent:
        """
        ...
        with self.strategy.scope():
            self.agent = DQNAgent(
                state_shape=state_shape, action_size=action_size, **agent_kwargs
            )
...
def setup_distributed_training(
    strategy_type: str = "mirrored", devices: Optional[list] = None
) -> DistributedTrainer:
    """
