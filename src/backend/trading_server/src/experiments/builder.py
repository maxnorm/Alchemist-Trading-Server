...
class ExperimentBuilder:
        ...
        if not currency_pairs:
            ...
            for pair in currency_pairs:
                if not isinstance(pair, str) or len(pair) != 6:
                    errors.append(
                        f"Invalid currency pair format: {pair} (expected 6 characters)"
                    )

        # Validate training mode
        if training_mode not in ["live", "historical"]:
            errors.append(
                f"Invalid training mode: {training_mode} (must be 'live' or 'historical')"
            )
        ...
        if not hyperparameters:
            ...
            if "hidden_layers" in hyperparameters:
                ...
                if not isinstance(hidden_layers, list) or not all(
                    isinstance(x, int) and x > 0 for x in hidden_layers
                ):
    ...
    def clone_experiment(
        ...
        # Apply modifications
        features = (
            modifications.get("features", original.features)
            if modifications
            else original.features
        )
        currency_pairs = (
            modifications.get("currency_pairs", original.currency_pairs)
            if modifications
            else original.currency_pairs
        )
        training_mode = (
            modifications.get("training_mode", original.training_mode)
            if modifications
            else original.training_mode
        )
        hyperparameters = (
            modifications.get("hyperparameters", original.hyperparameters.copy())
            if modifications
            else original.hyperparameters.copy()
        )
