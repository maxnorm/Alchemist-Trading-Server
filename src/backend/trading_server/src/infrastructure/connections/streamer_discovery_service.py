...
class StreamerDiscoveryService:
    ...
    def __init__(
        ...
        self._registered_streamers: Dict[str, Dict] = (
            {}
        )  # symbol -> {streamer, digits, token}
    ...
    def _discover_streamers(self):
        ...
        while self._running and not self._shutdown_event.is_set():
            ...
            if not self._discovery_socket:
                self._logger.error(
                    "Discovery socket unexpectedly closed - stopping discovery loop"
                )
                break
    ...
    def _process_tick_message(self, tick_data: Dict, topic: str):
        ...
        if digits is None:
            ...
            digits = int(digits)
            self._logger.debug(f"Using digits={digits} for {symbol} from tick data")
        ...
        with self._registration_lock:
            ...
            try:
                ...
                thread = threading.Thread(
                    target=streamer.receive_tick, daemon=True, name=f"streamer_{symbol}"
                )
                thread.start()
                self._logger.info(
                ...
                self._logger.error(
                    f"Failed to auto-register streamer for {symbol}: {e}", exc_info=True
                )
