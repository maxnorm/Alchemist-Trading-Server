...
class RLTradingAgent:
    def __init__(
        self, account: Account, learning_rate=0.001, discount_factor=0.95, epsilon=1.0
    ):
    ...
    def execute_action(self, action, pair):
        ...
        if action == 0:  # Do nothing
            ...
            if len(self.account.current_trade) == 0:
                self.account.send_order(OrderType.BUY, pair, 0.01)  # Minimum lot size
                return 0  # Initial reward

        elif action == 2:  # Sell
            if len(self.account.current_trade) == 0:
                self.account.send_order(OrderType.SELL, pair, 0.01)  # Minimum lot size
                return 0  # Initial reward
