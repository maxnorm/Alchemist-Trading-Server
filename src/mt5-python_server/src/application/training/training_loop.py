...
class TrainingLoop:
    ...
    def _update_adaptive_epsilon(self):
        ...
        recent_rewards = None
        if hasattr(self.metrics_tracker, 'step_rewards') and len(self.metrics_tracker.step_rewards) > 0:
        ...
        if experience_count > 0 and self.metrics_tracker.total_steps % 10 == 0:
            ...
            new_epsilon = self.agent.update_epsilon_adaptive(experience_count, recent_rewards)
    ...
    def _select_action(self, state: np.ndarray) -> int:
        ...
        action = self.agent.act(state, training=self.config.training_enabled, action_mask=action_mask)
    ...
    def _update_and_log_metrics(self, action: int, reward: float, current_balance: float, previous_balance: float):
        ...
        if action == 0:
            ...
            pair_symbol = (self.env.data_providers[pair_index].currency_pair.symbol 
                           if self.env.data_providers and pair_index is not None and pair_index < len(self.env.data_providers) 
                           else "UNKNOWN")
            action_name = action_names.get(action_type.value if hasattr(action_type, 'value') else action_type, "UNKNOWN")
        ...
        log_msg = (f"[Step {self.metrics_tracker.total_steps}] Action: {action} ({action_desc}) | "
                  f"Reward: {reward:.4f} | "
    ...
    def _log_training_params(self) -> None:
        ...
        try:
            params = {
                ...
                'n_pairs': len(self.env.data_providers) if self.env.data_providers else 0,
            }
