...
class TickContractValidator(IContractValidator):
    ...
    def validate(self, data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if spread > 0.01:  # 100 pips max for forex
            return False, f"Unrealistic spread: {spread:.6f} (> 0.01 / 100 pips)"
        ...
        if "receive_time" in data and data["receive_time"] is not None:
            ...
            if isinstance(data["datetime"], datetime) and isinstance(data["receive_time"], datetime):
        ...
        if "latency_seconds" in data and data["latency_seconds"] is not None:
            latency_error = self.validate_field_type(data["latency_seconds"], int, "latency_seconds")
            if latency_error:
        ...
        if "stale_age_seconds" in data and data["stale_age_seconds"] is not None:
            stale_age_error = self.validate_field_type(data["stale_age_seconds"], int, "stale_age_seconds")
            if stale_age_error:
        ...
        if "timestamp_source" in data and data["timestamp_source"] is not None:
            ...
            if data["timestamp_source"] not in valid_sources:
                return False, f"Field 'timestamp_source' must be one of {valid_sources}, got '{data['timestamp_source']}'"
