...
class DataProviderRegistry:
    ...
    def __init__(self):
        ...
        self._metadata: Dict[str, Dict] = {}  # Provider metadata (version, description, etc.)
        ...
        for name, provider in providers_copy.items():
            try:
                ...
                if not isinstance(features, list):
                    logger.warning(f"Provider '{name}' returned non-list from get_features()")
                    continue
                ...
                logger.debug(f"Discovered {len(features)} features from provider '{name}'")
            except Exception as e:
                logger.error(f"Error discovering features from provider '{name}': {e}", exc_info=True)
        
        logger.info(f"Discovered {len(all_features)} total features from {len(providers_copy)} providers")
        return all_features
    ...
    def check_health(self) -> Dict[str, bool]:
        ...
        for name, provider in providers_copy.items():
            try:
                ...
                logger.error(f"Error checking health for provider '{name}': {e}", exc_info=True)
