    ...
    def __post_init__(self):
        ...
        if self.action_type == ActionType.BUY:
            if self.stop_loss is not None and self.stop_loss >= self.entry_price:
                raise ValueError(f"Stop loss for BUY must be below entry price")
            if self.take_profit is not None and self.take_profit <= self.entry_price:
                raise ValueError(f"Take profit for BUY must be above entry price")
        elif self.action_type == ActionType.SELL:
            if self.stop_loss is not None and self.stop_loss <= self.entry_price:
                raise ValueError(f"Stop loss for SELL must be above entry price")
            if self.take_profit is not None and self.take_profit >= self.entry_price:
                raise ValueError(f"Take profit for SELL must be below entry price")
