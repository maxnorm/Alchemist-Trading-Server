...
from environments.slippage_models import SlippageModel, FixedSlippage, TransactionCosts
...
class HistoricalTradingEnv(BaseTradingEnv):
    ...
    def _calculate_equity(self, bar: pd.Series) -> float:
        ...
        if self.position.side == 'long':
            unrealized = (bar['bid'] - self.position.entry_price) * self.position.quantity
        else:
            unrealized = (self.position.entry_price - bar['ask']) * self.position.quantity
