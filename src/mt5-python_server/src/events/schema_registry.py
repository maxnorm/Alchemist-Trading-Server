...
class CanonicalEventSchema:
    ...
    receive_time: Optional[datetime] = (
        None  # UTC normalized, timezone-aware (transaction time)
    )
    timestamp_metadata: Dict[str, Any] = field(
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.receive_time is not None:
            result["receive_time"] = (
                self.receive_time.isoformat()
                if isinstance(self.receive_time, datetime)
                else str(self.receive_time)
            )
            ...
            if isinstance(receive_time_value, str):
                try:
                    receive_time = datetime.fromisoformat(
                        receive_time_value.replace("Z", "+00:00")
                    )
                except ValueError:
                    try:
                        receive_time = datetime.strptime(
                            receive_time_value, "%Y-%m-%d %H:%M:%S"
                        )
                    except ValueError:
                        receive_time = None
    ...
    def validate(self) -> tuple[bool, Optional[str]]:
        ...
        if self.timestamp_metadata is not None:
            if not isinstance(self.timestamp_metadata, dict):
                return (
                    False,
                    f"Invalid timestamp_metadata type: {type(self.timestamp_metadata)}",
                )
