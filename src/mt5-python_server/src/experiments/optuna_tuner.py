...
class OptunaHyperparameterTuner:
    ...
    def create_study(
        ...
        if not study_name:
            study_name = f"{experiment.name}_optuna_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        try:
            ...
            logger.info(f"Created Optuna study {study_id} for experiment {experiment_id}")
            return study_id
    ...
    def run_trials(
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            try:
                ...
                import random
                value = random.uniform(0.5, 2.0) if direction == 'maximize' else random.uniform(-2.0, -0.5)
    ...
    def get_best_params(self, study_id: int) -> Dict[str, Any]:
        ...
        return json.loads(study_data['best_params']) if isinstance(study_data['best_params'], str) else study_data['best_params']
    ...
    def get_trial_results(self, study_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for row in rows:
                (trial_number, params_json, value, state, metrics_json, created_at, completed_at) = row
    ...
    def _suggest_hyperparameters(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        return {
            ...
            'replay_buffer_size': trial.suggest_categorical('replay_buffer_size', [10000, 100000, 500000]),
            ...
            'epsilon_decay': trial.suggest_categorical('epsilon_decay', [5000, 10000, 50000]),
            
            # Target update frequency: [100, 1000, 5000]
            'target_update_freq': trial.suggest_categorical('target_update_freq', [100, 1000, 5000]),
        }
    ...
    def _get_study(self, study_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            (id_val, experiment_id, study_name, direction, metric, n_trials, status,
             best_trial_number, best_value, best_params, param_importance) = row
