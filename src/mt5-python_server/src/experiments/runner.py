...
class ExperimentRunner:
    ...
    def start_experiment(self, experiment_id: int) -> bool:
        ...
        with self._lock:
            ...
            if experiment.status != ExperimentStatus.CREATED:
                logger.error(f"Experiment {experiment_id} is not in 'created' status (current: {experiment.status.value})")
                return False
            ...
            try:
                ...
                data_providers = self._get_data_providers_for_pairs(experiment.currency_pairs)
                ...
                training_config = self._create_training_config(experiment.hyperparameters)
                ...
                if self.experiment_tracker and hasattr(trainer.training_loop, 'tracker'):
    ...
    def _run_training(self, trainer: LiveTrainer, experiment_id: int):
        ...
        try:
            ...
            with self._lock:
                ...
                if trainer.training_loop and hasattr(trainer.training_loop, 'is_running'):
                    ...
                    try:
                        ...
                        if experiment:
                            ...
                            if mlflow_run_id and self.experiment_tracker:
                                try:
                                    mlflow_model_uri = self.experiment_tracker.get_model_uri()
                                except Exception as e:
                                    logger.warning(f"Failed to get MLflow model URI: {e}")
                            
                            # Get training metrics if available
                            metrics = None
                            if trainer.training_loop and hasattr(trainer.training_loop, 'metrics_tracker'):
                                try:
                                    metrics = trainer.training_loop.metrics_tracker.get_metrics()
                                except Exception as e:
                            ...
                            logger.info(f"Registered model {model.id} (v{model.version}) for experiment {experiment_id}")
                    except Exception as e:
                        logger.error(f"Failed to register model for experiment {experiment_id}: {e}", exc_info=True)
                
                logger.info(f"Experiment {experiment_id} completed with status: {final_status.value}")
                
        except Exception as e:
            logger.error(f"Error in training for experiment {experiment_id}: {e}", exc_info=True)
            with self._lock:
                if experiment_id in self.running_experiments:
                    del self.running_experiments[experiment_id]
    ...
    def _create_training_config(self, hyperparameters: Dict[str, Any]) -> TrainingConfig:
        """
