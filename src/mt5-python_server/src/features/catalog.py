...
class FeatureCatalog:
        ...
        try:
            ...
            for feature in features:
                try:
                    ...
                    logger.error(f"Error storing feature '{feature.name}': {e}", exc_info=True)
                    continue
    ...
    def get_features_by_source(self, source: str) -> List[Feature]:
        ...
        try:
            ...
            logger.error(f"Error retrieving features by source '{source}': {e}", exc_info=True)
            return []
        finally:
            if conn:
                conn.close()
    ...
    def get_features_by_category(self, category: str) -> List[Feature]:
        ...
        try:
            ...
            logger.error(f"Error retrieving features by category '{category}': {e}", exc_info=True)
            return []
        finally:
            if conn:
                conn.close()
    ...
    def update_feature_availability(self, name: str, available: bool) -> None:
        ...
        try:
            ...
            logger.error(f"Error updating feature availability '{name}': {e}", exc_info=True)
            if conn:
                conn.rollback()
        finally:
            if conn:
