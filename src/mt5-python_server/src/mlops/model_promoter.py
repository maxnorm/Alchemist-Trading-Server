...
class ModelPromoter:
    ...
    def promote_to_staging(self, version: int, approver: str) -> bool:
        ...
        try:
            ...
            # Add metadata tags
            self._set_version_tags(version, {
                "staged_by": approver,
    ...
    def promote_to_paper(
        ...
        try:
            ...
            if model.stage != DBModelStage.STAGING:
                raise ValueError(f"Model must be in Staging stage, currently {model.stage.value}")
            ...
            try:
                ...
                self._set_version_tags(version_num, {
                    "paper_trading": "true",
            ...
            self.logger.info(f"Model {model_id} (v{model.version}) promoted to Paper by {approver}")
            return True
        ...
        try:
            ...
            if model_id and self.model_registry:
                ...
                if model.stage != DBModelStage.PAPER:
                    raise ValueError(f"Model must be in Paper stage, currently {model.stage.value}")
            ...
            if self.model_registry:
                current_prod_models = self.model_registry.get_models_by_stage(DBModelStage.PRODUCTION)
            ...
            try:
                ...
                self._set_version_tags(version_num, {
                    ...
                    "validation_sharpe": str(validation.metrics.get('sharpe_ratio', '')) if validation else '',
                    "validation_drawdown": str(validation.metrics.get('max_drawdown', '')) if validation else '',
                })
            except (ValueError, AttributeError):
                pass
            
            model_identifier = f"{model_id} (v{version_str})" if model_id else f"v{version_str}"
            self.logger.info(f"Model {model_identifier} promoted to Production by {approver}")
            return True
    ...
    def rollback_production(
        ...
        try:
            ...
            if current_prod:
                ...
                self._set_version_tags(int(current_prod.version), {
            ...
            self._set_version_tags(int(target.version), {
