...
class ModelRegistry:
    ...
    def register_model(
        ...
        try:
            ...
            cursor.execute(
                "SELECT id FROM models WHERE version = %s",
                (version,)
            )
            if cursor.fetchone():
            ...
            logger.info(f"Registered model {model_id} (version {version}) for experiment {experiment_id}")
    ...
    def get_model(self, model_id: int) -> Optional[Model]:
        ...
        try:
            ...
            # Parse row data
            (id_val, version, exp_id, stage_str, features_json, hyperparams_json,
             metrics_json, mlflow_uri, mlflow_run, paper_results_json,
             created_at, promoted_at, promoted_by) = row
            ...
            return Model(
                ...
                paper_trading_results=json.loads(paper_results_json) if paper_results_json else None,
    ...
    def get_model_by_version(self, version: str) -> Optional[Model]:
        ...
        try:
            ...
            logger.error(f"Error getting model by version {version}: {e}", exc_info=True)
            return None
        finally:
            if conn:
                conn.close()
    ...
    def get_models_by_stage(self, stage: ModelStage) -> List[Model]:
        ...
        try:
            ...
            logger.error(f"Error getting models by stage {stage.value}: {e}", exc_info=True)
            return []
        finally:
            if conn:
                conn.close()
