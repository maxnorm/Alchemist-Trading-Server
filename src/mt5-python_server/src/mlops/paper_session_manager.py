...
class PaperTradingSessionManager:
    ...
    def create_session(
        ...
        try:
            ...
            logger.info(f"Created paper trading session {session_id} for model {model_id}")
    ...
    def get_session(self, session_id: int) -> Optional[PaperSession]:
        ...
        try:
            ...
            (id_val, model_id, status, start_balance, current_balance,
             total_trades, winning_trades, pnl, sharpe_ratio, max_drawdown,
    ...
    def get_sessions_by_model(self, model_id: int) -> List[PaperSession]:
        ...
        try:
            ...
            logger.error(f"Error getting sessions for model {model_id}: {e}", exc_info=True)
            return []
        finally:
            if conn:
                conn.close()
    ...
    def validate_session(
        ...
        # Calculate win rate
        win_rate = session.winning_trades / session.total_trades if session.total_trades > 0 else 0.0
        
        # Validation checks
        checks = {
            'min_trades': session.total_trades >= criteria.get('min_trades', 0),
            'min_win_rate': win_rate >= criteria.get('min_win_rate', 0),
            'min_sharpe_ratio': (session.sharpe_ratio or 0) >= criteria.get('min_sharpe_ratio', 0),
            'max_drawdown': (session.max_drawdown or 1.0) <= criteria.get('max_drawdown', 1.0),
        }
