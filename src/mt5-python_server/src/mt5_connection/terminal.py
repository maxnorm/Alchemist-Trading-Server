...
class MT5Terminal(Connection):
    ...
    async def send_order(self, order_type, pair, lotsize, price=None, sl=None, tp=None, _retry=True):
        """
        ...
        try:
            ...
            with open(log_path, 'a') as f:
                f.write(json_log.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"D","location":"terminal.py:78","message":"send_order before send_msg","data":{"order_data":data},"timestamp":int(time.time()*1000)}) + '\n')
        except: pass
        # #endregion
        self.send_msg(json.dumps(data))
        ...
        try:
            ...
            payload = {"sessionId":"debug-session","runId":"run-debug","hypothesisId":"H1","location":"terminal.py:send_order","message":"after_send_msg","data":{"socket_fileno":self._Connection__socket.fileno() if hasattr(self,"_Connection__socket") else None,"order_type":order_type,"symbol":pair.symbol,"lotsize":lotsize},"timestamp":int(_t.time()*1000)}
            try:
        ...
        try:
            ...
            log_path = r'c:\Users\maxno\Desktop\Projet\1.1\.cursor\debug.log'
            with open(log_path, 'a') as f:
                f.write(json_log.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"D","location":"terminal.py:87","message":"send_order after send_msg, before get_response","data":{},"timestamp":int(time.time()*1000)}) + '\n')
        except: pass
        # #endregion
        ...
        try:
            ...
            try:
                import json as json_log, os, time as _t
                payload = {"sessionId":"debug-session","runId":"run-debug","hypothesisId":"H1","location":"terminal.py:send_order","message":"got_response","data":{"has_response":response is not None,"return_code":response.get('return_code') if isinstance(response,dict) else None},"timestamp":int(_t.time()*1000)}
                try:
                    host_log = r'c:\Users\maxno\Desktop\Projet\1.1\.cursor\debug.log'
                    with open(host_log, 'a') as f:
                        f.write(json_log.dumps(payload) + '\n')
            ...
            try:
                ...
                log_path = r'c:\Users\maxno\Desktop\Projet\1.1\.cursor\debug.log'
                with open(log_path, 'a') as f:
                    f.write(json_log.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"D","location":"terminal.py:99","message":"send_order got response","data":{"return_code":response.get('return_code'),"has_ticket":'ticket' in response,"comment":response.get('comment'),"full_response":str(response)},"timestamp":int(time.time()*1000)}) + '\n')
            except: pass
            # #endregion
            ...
            try:
                ...
                log_path = r'c:\Users\maxno\Desktop\Projet\1.1\.cursor\debug.log'
                with open(log_path, 'a') as f:
                    f.write(json_log.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"E","location":"terminal.py:108","message":"TimeoutError in get_response","data":{"error":str(e)},"timestamp":int(__import__('time').time()*1000)}) + '\n')
            except: pass
            # #endregion
            raise TimeoutError(
            ...
            try:
                ...
                log_path = r'c:\Users\maxno\Desktop\Projet\1.1\.cursor\debug.log'
                with open(log_path, 'a') as f:
                    f.write(json_log.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"E","location":"terminal.py:114","message":"ConnectionError in get_response","data":{"error":str(e)},"timestamp":int(__import__('time').time()*1000)}) + '\n')
            except: pass
            # #endregion
            # Retry once after a forced ping if allowed
            if _retry:
                await self._ensure_connection(force_ping=True)
                return await self.send_order(order_type, pair, lotsize, price, sl, tp, _retry=False)
        ...
        try:
            ...
            log_path = r'c:\Users\maxno\Desktop\Projet\1.1\.cursor\debug.log'
            with open(log_path, 'a') as f:
                f.write(json_log.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"D","location":"terminal.py:135","message":"Checking return_code","data":{"return_code":return_code,"expected_executed":TradeRequest.EXECUTED.value,"expected_placed":TradeRequest.PLACED.value,"comment":comment},"timestamp":int(time.time()*1000)}) + '\n')
        except: pass
        # #endregion
        
        # Accept both EXECUTED (10009) for market orders and PLACED (10008) for pending orders
        if return_code == TradeRequest.EXECUTED.value or return_code == TradeRequest.PLACED.value:
            trade = Trade(
                response['ticket'],
                order_type,
                pair,
            ...
            try:
                ...
                log_path = r'c:\Users\maxno\Desktop\Projet\1.1\.cursor\debug.log'
                with open(log_path, 'a') as f:
                    f.write(json_log.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"D","location":"terminal.py:149","message":"Trade created successfully","data":{"ticket":trade.ticket,"lotsize":trade.lotsize,"price":trade.open_price},"timestamp":int(time.time()*1000)}) + '\n')
            except: pass
            # #endregion
            return trade
        else:
            ...
            try:
                ...
                log_path = r'c:\Users\maxno\Desktop\Projet\1.1\.cursor\debug.log'
                with open(log_path, 'a') as f:
                    f.write(json_log.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"D","location":"terminal.py:167","message":"Invalid return_code, raising exception","data":{"return_code":return_code,"retcode_desc":retcode_desc,"comment":comment,"error_details":error_details},"timestamp":int(time.time()*1000)}) + '\n')
            except: pass
            # #endregion
