...
from utils.logging_config import get_structured_tick_streamer_logger, get_logger
...
class MT5TickStreamer:
    ...
    def __init__(self, socket, asset, stop_char='\n',
        ...
        self.__db_writer_thread = threading.Thread(target=self._db_writer_worker, daemon=True)
    ...
    def __validate_tick(self, symbol: str, date_time: str, ask: float, bid: float) -> bool:
        """
        ...
        if self.__asset.bid is not None and self.__asset.ask is not None:
            bid_change_pct = abs(bid - self.__asset.bid) / self.__asset.bid if self.__asset.bid > 0 else 0
            ask_change_pct = abs(ask - self.__asset.ask) / self.__asset.ask if self.__asset.ask > 0 else 0
    ...
    def __flush_ticks_to_db(self, ticks):
        ...
        try:
            with self.__logger.performance_context('flush_ticks_to_db', count=len(ticks)):
    ...
    def receive_tick(self):
        ...
        self.__logger.log_event(
            ...
            message="Starting tick reception loop"
        )
        cum_data = ''
        tick_count = 0
            ...
            while not self.__shutdown_flag.is_set():
                ...
                if (current_time - last_market_check).total_seconds() >= 300:  # Check every 5 minutes
                    ...
                    if market_open and self.__last_tick_time is not None:
                        time_since_last_tick = (current_time - self.__last_tick_time).total_seconds()
                        if time_since_last_tick >= 300 and not self.__no_tick_warning_logged:  # 5 minutes
                            self.__logger.log_event(
                                event_type='no_ticks_received',
                                message=f"No ticks received for {time_since_last_tick/60:.1f} minutes during market hours",
                                metrics={
                                    'minutes_since_last_tick': time_since_last_tick / 60,
                                    'market_open': True
                                },
                                level='WARNING'
                            )
