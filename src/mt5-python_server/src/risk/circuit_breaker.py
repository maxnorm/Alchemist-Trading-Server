...
class CircuitBreaker:
    ...
    def __init__(self, config: CircuitBreakerConfig = None, database: Any = None, price_history_provider: Any = None):
        """
    ...
    def initialize(self, starting_balance: float) -> None:
        ...
        with self._lock:
            ...
            self._reset_windows()
            self.logger.info(f"CircuitBreaker initialized with balance: {starting_balance}")
    ...
    def record_trade(self, pnl: float, balance: float = None) -> None:
        ...
        with self._lock:
            ...
            if balance is not None:
                self._metrics.starting_balance = max(self._metrics.starting_balance, balance)
            ...
            if self._state == CircuitBreakerState.HALF_OPEN:
                ...
                if pnl >= 0 and self._half_open_trades >= self.config.half_open_max_trades:
                    self._close()
    ...
    def record_latency(self, latency_ms: float) -> None:
        ...
        with self._lock:
            ...
            if self._latency_history:
                total = sum(l['latency_ms'] for l in self._latency_history)
                self._metrics.avg_latency_ms = total / len(self._latency_history)
    
    def record_volatility(self, volatility: float = None, price_history: List[float] = None) -> None:
        """
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'trip_time': self._metrics.trip_time.isoformat() if self._metrics.trip_time else None,
            }
    ...
    def _check_hourly_loss(self) -> tuple[bool, Optional[str]]:
        ...
        if loss_pct > self.config.max_loss_per_hour_pct:
            return False, f"Hourly loss exceeded: {loss_pct:.2%} > {self.config.max_loss_per_hour_pct:.2%}"
        return True, None
    ...
    def _check_daily_loss(self) -> tuple[bool, Optional[str]]:
        ...
        if loss_pct > self.config.max_loss_per_day_pct:
            return False, f"Daily loss exceeded: {loss_pct:.2%} > {self.config.max_loss_per_day_pct:.2%}"
        return True, None
    ...
    def _check_volatility(self) -> tuple[bool, Optional[str]]:
        ...
        if self._metrics.current_volatility <= 0 and self.price_history_provider:
            try:
                ...
                self.logger.warning(f"Failed to calculate volatility from price history: {e}")
        
        if self._metrics.current_volatility <= 0:
            return True, None
        
        vol_multiple = self._metrics.current_volatility / self.config.baseline_volatility
    ...
    def _check_error_rate(self) -> tuple[bool, Optional[str]]:
        ...
        error_rate = self._calculate_error_rate()
        if (self._metrics.operation_count >= self.config.min_operations_for_error_rate and
            error_rate > self.config.max_error_rate):
            return False, f"Error rate exceeded: {error_rate:.2%} > {self.config.max_error_rate:.2%}"
        return True, None
    
    def _check_latency(self) -> tuple[bool, Optional[str]]:
        """Check latency threshold"""
        if self._metrics.avg_latency_ms > self.config.max_latency_ms:
            return False, f"High latency: {self._metrics.avg_latency_ms:.0f}ms > {self.config.max_latency_ms}ms"
        return True, None
    ...
    def _cleanup_old_data(self) -> None:
        ...
        while self._error_history and self._error_history[0]['timestamp'] < error_window:
        ...
        while self._latency_history and self._latency_history[0]['timestamp'] < latency_window:
    ...
    def _extract_values(self, reason: str, breaker_type: str) -> tuple[Optional[float], Optional[float]]:
    ...
    def _log_trip_to_database(self, breaker_type: str, trigger_value: Optional[float], 
                              threshold_value: Optional[float], reason: str) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Logged circuit breaker trip to database: {breaker_type}")
            
        except Exception as e:
            self.logger.error(f"Failed to log circuit breaker trip to database: {e}", exc_info=True)
    ...
    def _update_database_resume(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Updated circuit breaker resume in database")
            
        except Exception as e:
            self.logger.error(f"Failed to update circuit breaker resume in database: {e}", exc_info=True)
            # Don't raise - allow operation to continue
