...
class KillSwitchEvent:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "reset_timestamp": (
                self.reset_timestamp.isoformat() if self.reset_timestamp else None
            ),
        }
...
class KillSwitch:
    ...
    def __init__(
        ...
        self.audit_log_path = (
            Path(audit_log_path)
            if audit_log_path
            else Path("logs/kill_switch_audit.json")
        )
    ...
    def arm(self) -> None:
        ...
        with self._lock:
            if self._state == KillSwitchState.TRIGGERED:
                self.logger.warning(
                    "Cannot arm - kill switch is triggered. Reset first."
                )
                return
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_event": (
                self._current_event.to_dict() if self._current_event else None
            ),
                ...
                with open(self.audit_log_path, "r") as f:
                    ...
                    for entry in data:
                        event = KillSwitchEvent(
                            ...
                            reset_timestamp=(
                                datetime.fromisoformat(entry["reset_timestamp"])
                                if entry.get("reset_timestamp")
                                else None
                            ),
                        )
