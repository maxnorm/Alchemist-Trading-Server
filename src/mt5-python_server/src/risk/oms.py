...
class OrderManagementSystem:
    ...
    def __init__(
        ...
        if self.db:
            try:
                ...
                self.logger.warning(f"Failed to load from database, falling back to file: {e}")
    ...
    def submit_order(self, order: Order) -> str:
        ...
        with self._lock:
            # Check idempotency
            if order.client_order_id and order.client_order_id in self._idempotency_keys:
                existing_id = self._idempotency_keys[order.client_order_id]
                self.logger.info(f"Idempotency hit: {order.client_order_id} -> {existing_id}")
                return existing_id
    ...
    def update_order_state(self, order_id: str, new_state: OrderState, **kwargs) -> Optional[Order]:
        """
        Update order state.
        ...
        with self._lock:
            ...
            self.logger.info(f"Order {order_id} state: {old_state.value} -> {new_state.value}")
            return order
        ...
        if critical_discrepancies and self.on_alert:
            for discrepancy in critical_discrepancies:
                alert_data = {
                    ...
                    'discrepancy_pct': abs(discrepancy.difference) / max(abs(discrepancy.local_quantity), abs(discrepancy.broker_quantity), 0.0001) if max(abs(discrepancy.local_quantity), abs(discrepancy.broker_quantity)) > 0 else 1.0,
    ...
    def _persist_order_to_db(self, order: Order) -> None:
        ...
        try:
            ...
            state_value = order.state.value.upper() if hasattr(order.state, 'value') else str(order.state).upper()
            ...
            self.logger.error(f"Failed to persist order to database: {e}", exc_info=True)
    ...
    def _load_from_db(self) -> None:
        ...
        try:
            ...
            for row in results:
                try:
                    ...
                    order = Order(
                        ...
                        created_at=row[17] if isinstance(row[17], datetime) else datetime.fromisoformat(str(row[17])),
                        updated_at=row[18] if isinstance(row[18], datetime) else datetime.fromisoformat(str(row[18])),
                        metadata=metadata
                    )
            ...
            self.logger.error(f"Failed to load orders from database: {e}", exc_info=True)
            raise
        ...
        try:
            ...
            for row in results:
                ...
                if abs(position.quantity) > 0.0001:
                    ...
                    position.average_price = float(avg_price) if avg_price else position.average_price
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            filled_orders = [o for o in self.orders.values() if o.state == OrderState.FILLED]
            
            return {
                'total_orders': len(self.orders),
                'active_orders': len(active_orders),
                'filled_orders': len(filled_orders),
                'rejected_orders': sum(1 for o in self.orders.values() if o.state == OrderState.REJECTED),
