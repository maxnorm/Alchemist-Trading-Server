...
class TradingController:
    ...
    def __init__(
        ...
        self.feed_stale_threshold = float(os.getenv('FEED_STALE_THRESHOLD_SECONDS', '180'))
    ...
    def start(self):
        ...
        if self.kill_switch and self.kill_switch.is_active():
            self.logger.critical("Cannot start - kill switch is active. Reset required.")
        ...
        try:
            while self.is_running:
                ...
                status = get_market_feed_status(self.env.data_providers, self.feed_stale_threshold)
                ready = status["ready"]
                ...
                if self._last_ready_state is None or self._last_ready_state != ready:
                    if hasattr(self.logger, 'log_event'):
                        ...
                        self.logger.info(f"Trading readiness changed: {'READY' if ready else 'NOT READY'} | {status}")
                ...
                if not ready:
                    if hasattr(self.logger, 'log_event'):
                        ...
                        self.logger.info(f"Market/feed not ready - skipping action execution {status}")
                    time.sleep(self.decision_interval)
                    continue
                        ...
                        if self.circuit_breaker and self.account:
                            ...
                            if self.env and hasattr(self.env, 'performance_metrics'):
                                ...
                                if volatility:
                                    ...
                                    daily_vol = volatility / np.sqrt(252) if volatility > 0.01 else volatility
                                    self.circuit_breaker.record_volatility(volatility=daily_vol)
                        
                        # Record equity snapshot if tracker is available
                        if self.equity_tracker and self.session_id and self.account:
                            if self.equity_tracker.should_record_snapshot(self.session_id):
                                try:
                                    current_balance = self.account.balance
                                    current_equity = self.account.equity if hasattr(self.account, 'equity') else current_balance
                                    ...
                                    self.logger.warning(f"Failed to record equity snapshot: {e}")
                    except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if action == 0:
                    ...
                    pair = self.env.data_providers[pair_index].currency_pair if self.env.data_providers and pair_index is not None else None
    ...
    def _check_and_reset_circuit_breaker(self) -> None:
        ...
        if self.circuit_breaker and self.account:
            if self._last_daily_reset is None or (now - self._last_daily_reset).days >= 1:
            ...
            # Hourly reset
            if self._last_hourly_reset is None or (now - self._last_hourly_reset).total_seconds() >= 3600:
    ...
    def _reconcile_positions(self) -> None:
        ...
        if (self._last_reconciliation is None or 
            (now - self._last_reconciliation).total_seconds() >= self.reconciliation_interval):
            ...
            try:
                ...
                if hasattr(self.account, 'get_positions'):
                    ...
                    for pos in positions:
                        symbol = getattr(pos, 'symbol', None) or getattr(pos, 'pair', None)
                        quantity = getattr(pos, 'volume', None) or getattr(pos, 'quantity', None)
                    ...
                    for trade_id, trade in self.account.current_trade.items():
                        ...
                        volume = getattr(trade, 'volume', None) or getattr(trade, 'quantity', None)
                ...
                if broker_positions:
                    ...
                    if discrepancies:
                        self.logger.warning(f"Position reconciliation found {len(discrepancies)} discrepancies")
                    else:
