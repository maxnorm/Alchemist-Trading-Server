...
class LiveTrainer:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Live trainer initialized - Training: {self.config.training_enabled}, "
    ...
    def start(self):
        ...
        self.logger.info(
            f"Training: {'ENABLED' if self.config.training_enabled else 'DISABLED'}"
        )
        self.logger.info(
            f"Trading: {'ENABLED' if self.config.trading_enabled else 'DISABLED'}"
        )
        self.logger.info(f"Decision Interval: {self.config.decision_interval}s")
        self.logger.info(
            f"Episode Duration: {self.config.episode_duration_hours} hours"
        )
        ...
        self.training_thread = threading.Thread(
            target=self.training_loop.run, daemon=True
        )
    ...
    def _execute_action(self, action: int, previous_balance: float) -> float:
        ...
        try:
            ...
            error_msg = (
                f"❌ ERROR executing action {action} ({action_name} on {symbol}) | "
        ...
        if (
            action_type == ActionType.BUY or action_type == ActionType.SELL
        ):  # Buy or Sell
            # New position opened
        ...
        reward = self.env.calculate_reward(
            ...
            ),  # Use action_type value for reward calculation
            has_position,
        ...
        if action_type == ActionType.BUY:  # Buy
            if not has_position:
                ...
                lot_size = (
                    ...
                    if self.account and entry_price
                    else 0.01
                )
                is_long = True
