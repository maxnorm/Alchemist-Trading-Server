...
def normalize_to_utc(dt: Union[datetime, str], source_timezone: Optional[pytz.BaseTzInfo] = None) -> datetime:
    """
...
def normalize_to_est(dt: Union[datetime, str], source_timezone: Optional[pytz.BaseTzInfo] = None) -> datetime:
    """
...
def parse_mt5_timestamp(timestamp_str: str, mt5_timezone_offset: Optional[float] = None) -> datetime:
    """
...
def parse_scraped_timestamp(timestamp_str: str, source_timezone: Optional[Union[str, pytz.BaseTzInfo]] = None) -> datetime:
    """
    ...
    if dt is None:
        raise ValueError(f"Failed to parse scraped timestamp '{timestamp_str}' with any known format")
