...
class TransactionCostModel:
    ...
    def calculate_spread_cost(
        ...
        if pair and pair.bid is not None and pair.ask is not None:
            ...
            spread_pct = actual_spread / pair.mid_price if pair.mid_price > 0 else self.spread_pct
        else:
    ...
    def calculate_commission(
        ...
        """
        # Commission on entry
        entry_commission = entry_price * self.commission_pct * lot_size * self.contract_size
        
        # Commission on exit
        exit_commission = exit_price * self.commission_pct * lot_size * self.contract_size
    ...
    def calculate_slippage(
        ...
        """
        # Base slippage
        base_slippage = entry_price * self.base_slippage_pct * lot_size * self.contract_size
    ...
    def calculate_entry_cost(
        ...
        return {
            ...
            'entry_cost_pct': total_entry_cost / (entry_price * lot_size * self.contract_size) if entry_price > 0 else 0.0
        }
    ...
    def calculate_exit_cost(
        ...
        spread_cost = self.calculate_spread_cost(exit_price, lot_size, not is_long, pair)
        exit_commission = exit_price * self.commission_pct * lot_size * self.contract_size
        slippage = self.calculate_slippage(exit_price, lot_size, volatility, not is_long)
        ...
        return {
            ...
            'exit_cost_pct': total_exit_cost / (exit_price * lot_size * self.contract_size) if exit_price > 0 else 0.0
        }
    ...
    def calculate_round_trip_cost(
        ...
        """
        entry_costs = self.calculate_entry_cost(entry_price, lot_size, is_long, pair, volatility)
        exit_costs = self.calculate_exit_cost(exit_price, lot_size, is_long, pair, volatility)
        ...
        return {
            ...
            'commission': entry_costs['entry_commission'] + exit_costs['exit_commission'],
    ...
    def calculate_cost_for_action(
        ...
        if action == 0:  # Hold - no cost
            ...
            costs = self.calculate_entry_cost(entry_price, lot_size, is_long, pair, volatility)
            ...
            costs = self.calculate_exit_cost(exit_price, lot_size, is_long, pair, volatility)
    ...
    def estimate_cost_percentage(
        ...
        """
        # Base costs
        base_cost_pct = self.spread_pct + (2 * self.commission_pct) + (2 * self.base_slippage_pct)
        
        # Add volatility adjustment
        if volatility is not None:
            normalized_vol = min(volatility, 0.05)
            volatility_adjustment = normalized_vol * self.volatility_slippage_factor * self.base_slippage_pct
            base_cost_pct += volatility_adjustment
        
        # Add size impact
        size_impact = np.log1p(lot_size * 10) * self.order_size_impact_factor * self.base_slippage_pct
