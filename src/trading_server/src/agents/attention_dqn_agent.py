...
class AttentionDQNAgent(DQNAgent):
    ...
    def __init__(
        ...
        self.use_cross_attention = self.attention_config.get(
    ...
    def _build_model(self) -> keras.Model:
        ...
        for i in range(n_lstm_layers - 1):
            x = layers.LSTM(
                lstm_units[i], return_sequences=True, activation=activation
            )(x)
        ...
        lstm_out = layers.LSTM(
            lstm_units[-1], return_sequences=True, activation=activation
        )(x)
