...
class DQNAgent:
    ...
    def __init__(
        ...
        if learning_rate_scheduler is not None:
            self.learning_rate_scheduler: Optional[BaseLearningRateScheduler] = (
    ...
    def _build_model(self) -> keras.Model:
        ...
        if len(lstm_units) < n_lstm_layers:
            lstm_units = lstm_units + [lstm_units[-1]] * (
                n_lstm_layers - len(lstm_units)
            )
        ...
        for i in range(n_lstm_layers):
            ...
            x = layers.LSTM(
                lstm_units[i], return_sequences=return_sequences, activation=activation
            )(x)
    ...
    def update_epsilon_adaptive(
        self, experience_count: int, recent_rewards: Optional[List[float]] = None
    ) -> float:
        """
        ...
        if (
            recent_rewards
            and len(recent_rewards) >= TradingConstants.PERFORMANCE_WINDOW_SIZE
        ):
            reward_variance = float(
                np.var(recent_rewards[-TradingConstants.PERFORMANCE_WINDOW_SIZE :])
            )
            avg_reward = float(
                np.mean(recent_rewards[-TradingConstants.PERFORMANCE_WINDOW_SIZE :])
            )

            # High variance or negative returns -> explore more
            if (
                reward_variance > TradingConstants.REWARD_VARIANCE_THRESHOLD_HIGH
                or avg_reward < 0
            ):
                target_epsilon = min(0.5, target_epsilon * 1.2)
            # Low variance and positive returns -> exploit more
            elif (
                reward_variance < TradingConstants.REWARD_VARIANCE_THRESHOLD_LOW
                and avg_reward > 0
            ):
        ...
        if self.use_double_dqn:
            ...
            for i in range(self.batch_size):
                if dones[i]:
                    ...
                    target_q_values[i][actions[i]] = (
                        ...
                        * next_q_values_target[i][next_actions[i]]
                    )
        else:
            ...
            for i in range(self.batch_size):
                if dones[i]:
                    ...
                    target_q_values[i][actions[i]] = (
                        rewards[i] + self.discount_factor * max_next_q_values[i]
                    )
    ...
    def load(self, filepath: str):
        ...
        """
        # Load Q-network
        self.q_network = keras.models.load_model(os.path.join(filepath, "q_network.h5"))
        ...
        self.target_update_freq = params.get(
            "target_update_freq", self.target_update_freq
        )
    ...
    def act_batch(
        ...
        for i, q_values in enumerate(q_values_batch):
            action_mask = (
                action_masks[i] if action_masks and i < len(action_masks) else None
            )
