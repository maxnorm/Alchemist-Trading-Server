...
class EconomicContractValidator(IContractValidator):
    ...
    def validate(self, data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if len(country) < 2 or len(country) > 3:
            return (
                False,
                f"Field 'country' must be 2-3 characters (ISO code), got '{country}' ({len(country)} chars)",
            )
        ...
        if len(event) > 250:
            return False, "Field 'event' exceeds maximum length of 250 characters"
        ...
        try:
            ...
            if impact < self.MIN_IMPACT or impact > self.MAX_IMPACT:
                return (
                    False,
                    f"Field 'impact' must be between {self.MIN_IMPACT} and {self.MAX_IMPACT}, got {impact}",
                )
        ...
        if "previous" in data and data["previous"] is not None:
            ...
            if len(previous_str) > 0:
                try:
                    ...
                    return (
                        False,
                        f"Field 'previous' must be a parseable number, got '{previous_str}'",
                    )
        ...
        if "consensus" in data and data["consensus"] is not None:
            ...
            if len(consensus_str) > 0:
                try:
                    ...
                    return (
                        False,
                        f"Field 'consensus' must be a parseable number, got '{consensus_str}'",
                    )
        ...
        if "actual" in data and data["actual"] is not None:
            ...
            if len(actual_str) > 0:
                try:
                    ...
                    return (
                        False,
                        f"Field 'actual' must be a parseable number, got '{actual_str}'",
                    )
