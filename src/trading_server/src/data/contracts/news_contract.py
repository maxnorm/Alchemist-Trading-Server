...
class NewsContractValidator(IContractValidator):
    ...
    def validate(self, data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if len(title) > self.MAX_TITLE_LENGTH:
            return (
                False,
                f"Field 'title' exceeds maximum length of {self.MAX_TITLE_LENGTH} characters",
            )
        ...
        if len(source) > self.MAX_SOURCE_LENGTH:
            return (
                False,
                f"Field 'source' exceeds maximum length of {self.MAX_SOURCE_LENGTH} characters",
            )
        ...
        if "content" in data and data["content"] is not None:
            ...
            if len(data["content"]) > self.MAX_CONTENT_LENGTH:
                return (
                    False,
                    f"Field 'content' exceeds maximum length of {self.MAX_CONTENT_LENGTH} characters",
                )
        ...
        if "impact" in data and data["impact"] is not None:
            ...
            if data["impact"] not in self.VALID_IMPACTS:
                return (
                    False,
                    f"Field 'impact' must be one of {self.VALID_IMPACTS}, got '{data['impact']}'",
                )
