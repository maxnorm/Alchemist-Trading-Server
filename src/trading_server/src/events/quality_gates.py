...
class EventQualityGates:
    ...
    def __init__(
        self,
        future_timestamp_buffer_seconds: Optional[float] = None,  # Make configurable
        stale_threshold_seconds: float = 300.0,
    ):
        """
        Initialize event quality gates

        :param future_timestamp_buffer_seconds: Buffer for future timestamp "
        "validation (default: 60s, configurable via env var)
    ...
    def _validate_timestamp(self, timestamp: Any) -> Tuple[bool, Optional[str]]:
        """
        ...
        if data_type == "tick":
            ...
            try:
                ...
                if high < open_price or high < close:
                    return False, "Invalid bar: high must be >= open and close"

                if low > open_price or low > close:
                    return False, "Invalid bar: low must be <= open and close"
