...
class ZeroMQBroker:
        ...
        with self._lock:
            ...
            try:
                ...
                try:
                    ...
                    self._logger.info(f"✓ PULL socket successfully bound to {pull_endpoint}")
                except Exception as e:
                    self._logger.error(f"✗ Failed to bind PULL socket to {pull_endpoint}: {e}", exc_info=True)
                    raise
                ...
                try:
                    ...
                    self._logger.info(f"✓ PUB socket successfully bound to {pub_endpoint}")
                except Exception as e:
                    self._logger.error(f"✗ Failed to bind PUB socket to {pub_endpoint}: {e}", exc_info=True)
                    raise
                ...
                try:
                    ...
                    pull_bound = self._pull_socket.getsockopt_string(zmq.LAST_ENDPOINT) if hasattr(zmq, 'LAST_ENDPOINT') else pull_endpoint
                    pub_bound = self._pub_socket.getsockopt_string(zmq.LAST_ENDPOINT) if hasattr(zmq, 'LAST_ENDPOINT') else pub_endpoint
                ...
                if self.verbose:
                    print(f"ZeroMQ broker started on ports {self.broker_port} (PULL) -> {self.tick_port} (PUB)")
                    print(f"EAs should connect to: tcp://{self.host}:{self.broker_port}")
                    print(f"Subscribers should connect to: tcp://{self.host}:{self.tick_port}")
    ...
    def _forward_loop(self):
        ...
        self._logger.info(f"PULL socket bound to tcp://*:{self.broker_port}, waiting for EA connections...")
        ...
        while self._running:
            try:
                ...
                if self._pull_socket in socks and socks[self._pull_socket] == zmq.POLLIN:
                    ...
                    try:
                        ...
                        if self._messages_forwarded < 5:
                            self._logger.info(f"✓ Received message from EA ({len(parts)} parts)")
                            # Log raw message preview for debugging
                            if len(parts) > 0:
                                preview = parts[-1][:200] if len(parts[-1]) > 200 else parts[-1]
                        ...
                        try:
                            ...
                            try:
                                ...
                                if self._errors % 100 == 0 or self._errors < 5:
                                    self._logger.warning(
                                        f"PUB socket buffer full (message {self._messages_forwarded}, error #{self._errors})"
                                    )
                                self._errors += 1
                                continue
                            
                            # Log first few messages and then periodically
                            if self._messages_forwarded <= 10 or self._messages_forwarded % 100 == 0:
                                self._logger.info(
                        ...
                        self._errors += 1
                        self._logger.error(f"Error forwarding message: {e}", exc_info=True)
            ...
            if now - last_stats_log >= stats_interval:
                with self._lock:
                    ...
                    if stats['messages_forwarded'] == 0 and stats['runtime_seconds'] > 120:
                        self._logger.warning(
        ...
        with self._lock:
            ...
            if self._pull_socket:
                try:
                    ...
                    self._logger.info(f"PULL socket: bound to tcp://*:{self.broker_port}")
                except Exception as e:
